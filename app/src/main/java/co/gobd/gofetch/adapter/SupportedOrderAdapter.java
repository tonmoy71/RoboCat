 ...
 */
public class SupportedOrderAdapter
    extends RecyclerView.Adapter<SupportedOrderAdapter.SupportedOrderViewHolder> {
  ...
  public SupportedOrderAdapter(final Context context, OrderService service) {
    ...
    orderService.getAllSupportedOrder(new SupportedOrderCallback() {
      @Override public void onReceiveOrderSuccess(List<SupportedOrder> orders) {
        itemList = orders;
        notifyDataSetChanged();
      }

      @Override public void onReceiveOrderFailure() {
  ...
  @Override public SupportedOrderViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    SupportedOrderViewHolder viewHolder = null;
    try {
      View layoutView =
          LayoutInflater.from(parent.getContext()).inflate(R.layout.cardview_supported_order, null);
  ...
  @Override public void onBindViewHolder(SupportedOrderViewHolder holder, int position) {
    ...
    if (!itemList.isEmpty()) {
      ...
      Picasso.with(context).load(itemList.get(position).getImageUrl()).into(holder.ivItemImage);
    }
  }
  ...
  public class SupportedOrderViewHolder extends RecyclerView.ViewHolder
