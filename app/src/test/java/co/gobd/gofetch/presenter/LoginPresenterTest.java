...
@RunWith(MockitoJUnitRunner.class) public class LoginPresenterTest {
  ...
  @Captor ArgumentCaptor<LoginCallback> loginCallbackArgumentCaptor;
  ...
  @Test public void shouldReturnTrueWhenValidCredentials() {
  ...
  @Test public void shouldStartActivityWhenSuccessfulLogin() {
  ...
  @Test public void shouldShowErrorWhenLoginFail() {
    when(view.getUserName()).thenReturn("abcde");
    when(view.getPassword()).thenReturn("123456");
    presenter.login();
    verify(service).login(anyString(), anyString(), loginCallbackArgumentCaptor.capture());
